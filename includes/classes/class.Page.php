	  ...
	  */	   
	  function get_first_page_nav($user_link="",$link_para="")
      {
        ...
        if(!$this->is_first_page()&& $this->show_first_last)
            return ' <a href="'.$this->page_name.'?page=0'.$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
      ...
      function get_last_page_nav($user_link="",$link_para="")
      {
        ...
        if(!$this->is_last_page()&& $this->show_first_last)
            return ' <a href="'.$this->page_name.'?page='.($this->total_page-1).$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
        elseif($this->show_first_last && $this->show_disabled_links)
            return $user_link;
      }
      function get_next_page_nav($user_link="",$link_para="")
      {
        ...
        if(!$this->is_last_page()&& $this->show_prev_next)
			return ' <a href="'.$this->page_name.'?page='.($this->page+1).$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
      ...
      function get_scroll_prev_page_nav($user_link="",$link_para="")
      {
        ...
        if($this->page>$this->scroll_page &&$this->show_scroll_prev_next)
            return ' <a href="'.$this->page_name.'?page='.($this->page-$this->scroll_page).$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
      ...
      function get_number_page_nav($link_para="")
      {
        ...
        for(;$i<$j+$scroll_page && $i<$this->total_records;$i++)
        {
         if($i==$this->page)
            ...
            return ' <a href="'.$this->page_name.'?page='.$i.$this->qry_str.'" '.$link_para.'>'.($i+1).'</a> ';
        }
      }
      ...
      function get_page_nav()
      {
        ...
        $this->calculate();
		$PageNav	= '';
        $PageNav	.= $this->get_first_page_nav("",$this->link_para);
        $PageNav	.= $this->get_scroll_prev_page_nav("",$this->link_para);
        $PageNav	.= $this->get_prev_page_nav("",$this->link_para);
        //***$PageNav	.= $this->get_number_page_nav($this->link_para);
        $PageNav	.= $this->get_next_page_nav("",$this->link_para);
        $PageNav	.= $this->get_scroll_next_page_nav("",$this->link_para);
        $PageNav	.= $this->get_last_page_nav("",$this->link_para);
		return $PageNav;
