  ...
  */
  ...
  private function EncodeFile($path, $encoding = 'base64') {
    try {
      ...
      if (PHP_VERSION < 5) { // changed the version from 6 to 5 by harika on 16-09-2011
        $magic_quotes = get_magic_quotes_runtime();
        set_magic_quotes_runtime(0);
      }
      $file_buffer  = file_get_contents($path);
      $file_buffer  = $this->EncodeString($file_buffer, $encoding);
      if (PHP_VERSION < 5) { set_magic_quotes_runtime($magic_quotes); } // changed the version from 6 to 5by harika on 16-09-2011
      return $file_buffer;
    } catch (Exception $e) {
      $this->SetError($e->getMessage());
      return '';
